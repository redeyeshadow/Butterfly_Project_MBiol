...
Long_Lats_Years %>%
  ...
  geom_point(aes( x = decimalLongitude, y = decimalLatitude,
                  col = is.na(July)))  ## Just weird coastal ones.
```
...
Months<- c('August','July','June','May' ,'April','March','SeptBefore')
...
VC_Seasonal_Temps %>% 
  ...
  select( VCNUMBER,VCNAME, Year, Month, MeanTemp ) -> VC_Monthly_Temps_Long
...
UA_Monthly_Temps %>% 
  ...
  separate(MidDate     ,
            into = c('Year', 'Month', 'MidDay' ),
           convert = TRUE, remove = FALSE)  -> UA_Monthly_Temps_Long
...
MonthlyTemps %>%
  select(SeptBefore = `9`)%>%
  mutate(Year = Year+1) -> SeptBefore


MonthlyTemps %>% 
  left_join(SeptBefore,
  ...
  select( T_August   =   `8` ,
          ...
          T_April    =  `4`,
          T_March    =   `3`  ,
          T_SeptBefore = SeptBefore )-> Temps_perLocYear
