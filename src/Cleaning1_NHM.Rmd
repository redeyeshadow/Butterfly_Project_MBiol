...
NHM_filter2 %>%
  ...
  mutate( Implausible = (`right_wing (mm)` < minimum_plausible) | (`right_wing (mm)` > maximum_plausible))   ->NHM_filter3x

NHM_filter3x %>%
  filter( `Taxon name` == 'Aglais urticae') %>%
  select( `right_wing (mm)` , month, maximum_plausible, minimum_plausible)%>%
  ggplot()+
  geom_point(aes( x = month, y = `right_wing (mm)` ))+
  geom_point(aes( x = month, y =  minimum_plausible), col = 'red')+
 geom_point(aes( x = month, y =  maximum_plausible), col = 'red')+
  ggtitle('Size distribution of Aglais urticae by month')
