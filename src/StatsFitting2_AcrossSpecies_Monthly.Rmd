---
title: "StatsFitting2_AcrossSpecies_NHM"
author: "Chris Terry"
date: "`r Sys.Date()`"
output: 
  html_document: 
...
Main document for making main text results for the NHM data. 
...
save(AverageYearEffect,file= '../TopLevelModelFits/Average_YearEffect_Model')
...
save(MS, file='../TopLevelModelFits/Size_YearEffect_Model')
save(MF, file='../TopLevelModelFits/FamilyGroup_YearEffect_Model')
```

```{r}
load('../TopLevelModelFits/Size_YearEffect_Model')
load('../TopLevelModelFits/FamilyGroup_YearEffect_Model')
load('../TopLevelModelFits/Average_YearEffect_Model')
```
...
Trends %>%
  ...
  brm( mean|mi(sd)  ~   RedListListed      ,
       data = .) -> RedListListed_YearEffect_Model

save(Generalist_YearEffect_Model, file = '../TopLevelModelFits/Generalist_YearEffect_Model')
save(RedListListed_YearEffect_Model, file = '../TopLevelModelFits/RedListListed_YearEffect_Model')
```

```{r}
load('../TopLevelModelFits/Generalist_YearEffect_Model')
load('../TopLevelModelFits/RedListListed_YearEffect_Model')

Generalist_YearEffect_Model %>% posterior_summary()
RedListListed_YearEffect_Model %>% posterior_summary()
...
save( RedList_TempEffect_Model, file = '../TopLevelModelFits/RedList_TempEffect_Model')

```

```{r}
load( '../TopLevelModelFits/RedList_TempEffect_Model')

RedList_TempEffect_Model %>% posterior_summary()

```

## Other non-significant predictors size (/family)  + specialism
...
Trends %>%
  filter( variable == 'b_Temp' ) %>%
  brm( mean|mi(sd)  ~   MeanRightWing      ,
       data = .) -> Size_TempEffect_Model

save(Generalist_TempEffect_Model, file = '../TopLevelModelFits/Generalist_TempEffect_Model')
save(Size_TempEffect_Model, file = '../TopLevelModelFits/Size_TempEffect_Model')
```

```{r}
load('../TopLevelModelFits/Generalist_TempEffect_Model')
load('../TopLevelModelFits/Size_TempEffect_Model')

Generalist_TempEffect_Model %>% posterior_summary()
Size_TempEffect_Model %>% posterior_summary()
...
bTemp_pred_NHM %>%
  brm( median|mi(sd)  ~  1,
       data = ., iter = 1000)%>%
  add_criterion(criterion = c("loo")) -> bTemp_model_NHM_AV
