...
```{r}
## Cycle through each species to fit models (could go faser by not compiling each time)

for( Sp in Species_list){
  print(Sp)
  Filt_BF %>%
    filter(`Taxon Name` ==Sp ) %>%
    brm( wingspan_mm ~  year +  SummerTemp_YearBefore ,
         data = ., refresh = 0) -> MODEL_FIT
  save(MODEL_FIT, file = paste('../NHM_modelfits1/',Sp  ))
}

## Map over each  each species to fit models
...
map_df(Species_list, function(Sp){
  ...
  load(file = paste('../NHM_modelfits1/',Sp  )) 
  MODEL_FIT %>%
    as_draws_array %>%
...
```{r}
Posterior_Summaries %>%
  filter( variable == 'b_SummerTemp_YearBefore')
